...
export function AllJobsPage() {
  ...
  const handleKill = async (id: string) => {
    // Kill is not supported from the jobs list (requires a run ID).
    // The JobTable renders a kill button for running jobs but from this
    // page we do not have a specific run ID to target, so we show a toast.
    toast.info("Use the job detail page to kill a specific run");
  };

  const handleEnable = async (id: string) => {
    try {
      await enableJob(id);
      toast.info("Job enabled");
    } catch (err) {
      toast.error(
