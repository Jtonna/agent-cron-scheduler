...
export function JobForm({ job, title, onSubmit, submitLabel = "Save", onCancel }: JobFormProps) {
  const s = useJobFormState(job ? { job } : undefined);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!s.validate()) {
      ...
      // to that tab so the user can see them.
      const timeoutNum = typeof s.timeoutSecs === "string" ? parseFloat(s.timeoutSecs) : s.timeoutSecs;
      const hasAdvancedError = isNaN(timeoutNum) || timeoutNum < 0;
