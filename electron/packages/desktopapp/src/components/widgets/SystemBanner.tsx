 ...
 * SystemBanner
 *
 * Pill-shaped banner that surfaces uptime, backup status, and the
 * current version. When `updateAvailable` is set the right side
 * morphs into an "Update" CTA + dismiss; otherwise it shows the
...
export function SystemBanner({
  ...
  return (
    <div
      className={`flex items-center h-[var(--height-banner)] px-5 bg-surface-secondary border rounded-pill text-sm text-fg-muted ${showUpdate ? "border-brand-ring" : "border-border"}`}
    >
      {/* Left — uptime & backups */}
