  ...
  return (
    <div className="border-b border-border-subtle bg-surface-secondary">
      ...
      <div className="px-16 pt-3 flex items-center justify-between">
        <span className="text-xs font-semibold text-fg-subtle uppercase tracking-wider">Filters</span>
      ...
      <div className="px-16 py-4 flex items-end gap-6">
        <div className="flex flex-col gap-1.5">
          <span className="text-xs font-semibold text-fg-muted uppercase tracking-wider">Cost ($)</span>
          <div className="flex items-center gap-2">
            <FilterInput label="Minimum cost" placeholder="Min" value={filters.costMin ?? ""} onChange={(v) => onChange({ ...filters, costMin: v || undefined })} />
            <span className="text-fg-faint text-xs">–</span>
            <FilterInput label="Maximum cost" placeholder="Max" value={filters.costMax ?? ""} onChange={(v) => onChange({ ...filters, costMax: v || undefined })} />
          </div>
        </div>

        <div className="flex flex-col gap-1.5">
          <span className="text-xs font-semibold text-fg-muted uppercase tracking-wider">Duration (sec)</span>
          <div className="flex items-center gap-2">
            <FilterInput label="Minimum duration" placeholder="Min" value={filters.durationMin ?? ""} onChange={(v) => onChange({ ...filters, durationMin: v || undefined })} />
            <span className="text-fg-faint text-xs">–</span>
            <FilterInput label="Maximum duration" placeholder="Max" value={filters.durationMax ?? ""} onChange={(v) => onChange({ ...filters, durationMax: v || undefined })} />
          </div>
        </div>

        <div className="flex flex-col gap-1.5">
          <span className="text-xs font-semibold text-fg-muted uppercase tracking-wider">Date range</span>
          <div className="flex items-center gap-2">
            <TextField aria-label="Date from" value={filters.dateFrom ?? ""} onChange={(v) => onChange({ ...filters, dateFrom: v || undefined })}>
              <Input type="date" className="h-[var(--height-input)] px-2.5 text-sm border border-border rounded-input bg-surface outline-none focus:border-brand-ring transition-colors" />
            </TextField>
            <span className="text-fg-faint text-xs">–</span>
            <TextField aria-label="Date to" value={filters.dateTo ?? ""} onChange={(v) => onChange({ ...filters, dateTo: v || undefined })}>
              <Input type="date" className="h-[var(--height-input)] px-2.5 text-sm border border-border rounded-input bg-surface outline-none focus:border-brand-ring transition-colors" />
            </TextField>
          </div>
        </div>
